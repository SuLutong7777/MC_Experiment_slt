...
def get_rays_full_image_use_camera(
    ...
    intrinsics_inv = torch.inverse(intrinsic[:3, :3]) if not intrinsic is None else torch.inverse(camera_model.get_intrinsic()[:3, :3])
...
def get_rays_kps_use_camera(H,
    ...
    intrinsics_inv = torch.inverse(camera_model.get_intrinsic()[idx_in_camera_param][:3, :3])
