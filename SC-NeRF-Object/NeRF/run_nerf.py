...
def train():
    ...
    ######################## 加载数据集 ########################
    (
        images, noisy_extrinsic, render_poses, H, W, noisy_focals, i_split, gt_camera_info, depth_info_tensor
    ) = load_blender_data(args.datadir, args.half_res, args, args.testskip)    
    
    print("Loaded blender dataset")
    print("Images shape: {}".format(images.shape))
    print("HW: ", H, W)
    print("noisy_focals: ", noisy_focals.shape)
    print("noisy_extrinsic: ", noisy_extrinsic.shape)
    print("render_poses: ", render_poses.shape)
    print("depth_info_tensor: ", depth_info_tensor.shape)
    ...
    noisy_train_intrs = gt_intrinsic[i_train]
    print("noisy_train_poses: ", noisy_train_poses.shape)
    print("noisy_train_intrs: ", noisy_train_intrs.shape)
    ...
    H, W =int(H), int(W)
    noisy_initial_intrinsic = torch.tensor(gt_intrinsic)
    ...
    (
        ...
        args, noisy_train_intrs, noisy_train_poses, H, W, mode="train", device=device
    )
    ...
    noisy_focals = torch.tensor(noisy_focals).to(device)
    ...
    for i in trange(start, N_iters):
        ...
        # target_shape:  torch.Size([260, 390, 3])
        target = images[img_i].to(device)
        noisy_pose = noisy_extrinsic[img_i, :3, :4].to(device)
        # 获取该图片对应的深度图 [260, 390]
        depth_info = depth_info_tensor[img_i].to(device)
        ...
        if N_rand is not None:
            if i < args.precrop_iters:
                ...
                # print("img_coords_shape: ", img_coords.shape)
                coords = img_coords

            coords = torch.reshape(coords, [-1, 2]).to(device)
            ...
            select_inds = torch.tensor(np.random.choice(
                coords.shape[0], 
                size=[N_rand], 
                replace=False  # 不允许重复选择
            )).to(device)
            
            select_coords = coords[select_inds].long()  # [N_rand, 2]

            # 检查每个选择的像素点深度是否为 0，如果为 0 则重新选择该点
            for idx in range(N_rand):
                while True:
                    # 获取当前像素点的深度
                    depth_value = depth_info[select_coords[idx, 1], select_coords[idx, 0]]
                    if depth_value != 0:
                        break  # 如果深度不为 0，跳出循环，继续下一个点
                    else:
                        # 重新选择该像素点
                        new_ind = np.random.choice(coords.shape[0], size=1, replace=True)
                        select_coords[idx] = coords[new_ind].long()  # 替换为新的坐标
            # 获取选择的坐标 [1024, 2]
            ...
            # (N_rand, 3)
            target_s = target[select_coords[:, 1], select_coords[:, 0]].to(device)

        #####  Core optimization loop  #####
        rgb, disp, acc, extras = render(
            H=H, W=W, chunk=args.chunk, rays=batch_rays, 
            verbose=i < 10, retraw=True, camera_model=camera_model, idx_in_camera_param=img_i_train_idx,
        ...
        test_render = (i % args.i_testset == 0 and i > 80000) or args.debug
        ...
        if not camera_model is None and test_render:
            ...
            gt_transformed_pose_test = align_ate_c2b_use_a2b(
                gt_extrinsic[i_train].cuda(), 
                camera_model.get_extrinsic().detach().cuda(), 
                gt_extrinsic[i_test].cuda()
            )
        ...
        if val_render:
            ...
            with torch.no_grad():
                if camera_model is None:
                    ...
                    rgb, disp, acc, extras = render(
                        H=H, W=W, chunk=args.chunk, gt_intrinsic=gt_intrinsic.cuda(),
                        gt_extrinsic=gt_extrinsic.cuda(), mode="val", i_map=i_val,
...
if __name__ == '__main__':
    setproctitle.setproctitle("slt-sc-stump-pca_val=test")
