...
def render(
    ...
    camera_model=None, image_idx=None, i_map=None, idx_in_camera_param=None, gt_intrinsic=None, 
    ...
    if not rays is None:
        ...
        rays_o, rays_d = get_rays_full_image_use_camera(
            ...
            intrinsic=gt_intrinsic[image_idx]
        )
    ...
    if ndc and camera_model is None:
        ...
        rays_o, rays_d = ndc_rays_camera(H, W, camera_model, 1., rays_o, rays_d, intrinsic=gt_intrinsic[image_idx], idx_in_camera_param=idx_in_camera_param, mode =mode)
...
# def ndc_rays_camera(H, W, camera_model, near, rays_o, rays_d, intrinsic=None, idx_in_camera_param = None, mode="train"):
#     # Shift ray origins to near plane
#     t = -(near + rays_o[...,2]) / rays_d[...,2]
#     rays_o = rays_o + t[...,None] * rays_d
    
#     focal_x = camera_model.get_intrinsic()[idx_in_camera_param][0][0]
#     focal_y = camera_model.get_intrinsic()[idx_in_camera_param][1][1]
...
def ndc_rays_camera(H, W, camera_model, near, rays_o, rays_d, intrinsic, idx_in_camera_param, mode=None):
    # 小常数，用于避免除零或极小值问题
    eps = 1e-8

    # Shift ray origins to near plane
    t = -(near + rays_o[..., 2]) / (rays_d[..., 2] + eps)
    rays_o = rays_o + t[..., None] * rays_d
    ...
    # Projection
    o0 = -1. / (W / (2. * focal_x + eps)) * rays_o[..., 0] / (rays_o[..., 2] + eps)
    o1 = -1. / (H / (2. * focal_y + eps)) * rays_o[..., 1] / (rays_o[..., 2] + eps)
    o2 = 1. + 2. * near / (rays_o[..., 2] + eps)

    d0 = -1. / (W / (2. * focal_x + eps)) * ((rays_d[..., 0] / (rays_d[..., 2] + eps)) - (rays_o[..., 0] / (rays_o[..., 2] + eps)))
    d1 = -1. / (H / (2. * focal_y + eps)) * ((rays_d[..., 1] / (rays_d[..., 2] + eps)) - (rays_o[..., 1] / (rays_o[..., 2] + eps)))
    d2 = -2. * near / (rays_o[..., 2] + eps)
