...
def intrinsic_param_to_K(intrinsics):
    ...
    intrinsic_mat = torch.eye(4, 4)
    intrinsic_mat = intrinsic_mat[None, :, :].repeat(intrinsics.shape[0], 1, 1)
    for i in range(intrinsics.shape[0]):
        intrinsic_mat[i][0][0] = intrinsics[i][0]
        intrinsic_mat[i][1][1] = intrinsics[i][1]
        intrinsic_mat[i][0][2] = intrinsics[i][2]
        intrinsic_mat[i][1][2] = intrinsics[i][3]
    # intrinsic_mat[[0, 1, 0, 1], [0, 1, 2, 2]] = intrinsics
    intrinsic_mat = intrinsic_mat.to(device)
